...
- [ ] **UI-01**: The Setup screen's `Lafa` heading uses either the generated icon
      mark plus token-styled text, or a token-styled text heading if no vector
      wordmark is available (no wordmark is invented).
- [ ] **UI-02**: Setup, Quiz, Results, both feedback modals, `OfflinePill`, and
...
- [ ] **VALID-01**: A brand validation script/check confirms: `app.json` has no Expo
...
- [ ] **VALID-02**: `npm test -- src/theme/tokens.test.ts`, `npm run typecheck`, and
      `npm run lint` all pass after the palette/config changes.
- [ ] **VALID-03**: Manually verified on an EAS release/preview build (not Expo
